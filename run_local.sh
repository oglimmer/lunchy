...
lunchy {Source:\"mvn\", Default-Type:\"local\", Version-Info: \"Max Java 1.8\"}
  ...
  -t lunchy:docker:[TAG] #docker based build, default tag: latest, uses image http://hub.docker.com/_/maven
mysql {Source:\"mysql\", Default-Type:\"docker:5\", Version-Info: \"Max Mysql 5.x\"}
  ...
  -t mysql:docker:[TAG] #start docker, default tag 5, uses image http://hub.docker.com/_/mysql
...
if [ "$SKIP_HASH_CHECK" != "YES" ]; then
	if which md5 1>/dev/null; then
		declare SELF_HASH_MD5="be0809b2b91a892741ce75f0eaef4dcf"
...
if [ "$START_TOMCAT" = "YES" ]; then
	...
	if [ "$TYPE_SOURCE_TOMCAT" == "docker" ]; then
		...
		if [ ! -f ".tomcatPid" ]; then
			...
			verbosePrint "docker run --rm -d ${dockerAddLibRefs[@]} -p 8080:8080 --net=lunchynet --name=tomcat $ADD_HOST_INTERNAL   -v "$(pwd)/localrun/webapps":/usr/local/tomcat/webapps tomcat:$TYPE_SOURCE_TOMCAT_VERSION"
